...
//
// Modified by Boofdev and released under the GPLv3 license
...
func Main(conf f.TarmountConfig) {
	// Parse command-line arguments
	tarPath := conf.TarPath
	format := conf.Format
	mountPoint := conf.MountPoint

	// Create the tar file system
	root, err := NewTarCompressedTree(tarPath, format)
	if err != nil {
		log.Fatalf("Failed to create tar file system: %v", err)
	}
	...
	if err != nil {
		log.Fatalf("Failed to mount FUSE server: %v", err)
	}
	server.Serve()
}
